...
def count_empty_seats(chart)
  ...
  chart.flatten.select {|seat| seat == nil}.length #chart is a nested array hence the use of flatten method
end
...
def find_favorite(array_of_hash_objects)
  ...
  returned_hash = array_of_hash_objects.select {|hash| hash[:is_my_favorite?] == true} #returns array that contains hash
  returned_hash.empty? ? nil : returned_hash[0]
end
